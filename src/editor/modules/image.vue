...
export default {
    ...
    computed: {
		...
		maxFilesize () {
			return this.dropzoneOptions.maxFilesize
		}
    },
    ...
    methods: {
        ...
        fileAdded (file) {
			...
			const maxFilesize = this.dropzoneOptions.maxFilesize
			// console.log(fileSize.toFixed(1))
			// console.log(this.dropzoneOptions)
			if (+fileSize > +maxFilesize) {
				console.log(file)
				this.$refs.dropzone.removeFile(file)
				this.$modal.show('dialog', {
					text: 'Max filesize ' + this.maxFilesize + 'MB',
					buttons: [
						{
							title: 'Close'
						}
